...
Set **Map size** independently from **Wall width**. Set a hole diameter or label text/size/height first, click **Hole** or **Label**, then click an open face in the 3D view. Preview, Escape/Cancel, Undo, selected removal and label text/size/height edits are available. **Text height above base (mm)** sets each label’s independent rise (0.01–200 mm, default 0.8 mm, including older saved labels). Use **Update selected label** to apply edits. Labels use bundled licensed glyph outlines; counters remain open. Placements follow source-relative coordinates when map size changes while their dimensions stay in mm. Unsupported text and risky edges/walls/connectors are rejected. Printability depends on your chosen dimensions and printer.
