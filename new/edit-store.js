...
import {EMBOSS} from './customize.js';
const key='shp2stl.placements.v1';
let saved={};try{saved=JSON.parse(localStorage.getItem(key)||'{}');}catch{}
if(!saved||typeof saved!=='object'||Array.isArray(saved))saved={};
const memory=new Map();
const sourceKey=id=>id.slice(id.lastIndexOf('#')+1);
export function editsFor(id){const v=saved[sourceKey(id)];return Array.isArray(v)?structuredClone(v).map(e=>e.kind==='label'&&e.height===undefined?{...e,height:EMBOSS}:e):[];}
