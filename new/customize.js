...
export function textHeight(edit) {
  const height=edit.height===undefined?EMBOSS:edit.height;
  if(!Number.isFinite(height)||height<.01||height>200)throw Error('Text height above base must be between 0.01 and 200 mm');
  return height;
}
...
export async function editShape(edit,size) {
  ...
  if(edit.kind==='label')textHeight(edit);
...
export async function customize(regions,edits,options) {
  ...
  for(const edit of edits) {
    ...
    if(edit.kind==='hole')holes=pc.union(holes,shape);else {labels=pc.union(labels,shape);labelShapes.push(pc.union(shape));}
  }
  const layers=regions.layers.map(p=>holes.length?pc.difference(p,holes):p);
  return {...regions,layers,caps:undefined,holes,labels,labelShapes,expectedCuts:holes,raised:labels.length?pc.union(layers[2],labels):null};
}
